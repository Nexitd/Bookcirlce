...
export const ClubCard: FC<IClubCard> = ({ data, onClick }) => {
    return <div className="club__card" onClick={() => onClick(data.id)}>
        <div className="club__card_head">
            <img src={data.image} alt="" />

        </div>
        <div className="club__card_body">
            <h2 className="club__card_title">{data.title}</h2>

            <div className="club__card_tags">
                <ColorableTag text={data.meeting_type} icon={<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div className="club__card_tags-item">
                    {data.category.map(el => (<ColorableTag text={el} key={el} />))}
                </div>
            ...
            <p className="club__card_text">{data.description.length > 100 ? `${data.description.slice(0, 100)}...` : data.description}</p>
        </div>
    </div>
} 